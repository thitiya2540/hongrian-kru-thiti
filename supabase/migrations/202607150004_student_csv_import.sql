-- ห้องเรียนครูธิติ · CSV student import
-- Import is intentionally all-or-nothing so teachers never get a partially saved class.
...
begin
  ...
  if exists (
    ...
    group by trim(item ->> 'studentCode')
    having count(*) > 1
  ) then
    raise exception 'csv_duplicate_student_code';
  end if;

  if exists (
    select 1
    from jsonb_array_elements(p_rows) item
    join public.students s on s.student_code = trim(item ->> 'studentCode')
  ) then
    raise exception 'csv_student_code_already_exists';
  end if;
